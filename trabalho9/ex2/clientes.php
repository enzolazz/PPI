...
class Cliente
{
  ...
  function __construct($nome, $email, $cpf, $senha, $cep, $rua, $bairro, $cidade, $estado)
  {
    $this->nome = $nome;
    $this->email = $email;
    $this->cpf = $cpf;
...
function carregaClientesDeArquivo()
{
  ...
  while (!feof($arq)) {
    ...
    if ($cliente != "") {
      ...
      list($nome, $email, $cpf, $hashSenha, $cep, $rua, $bairro, $cidade, $estado) = array_pad(explode(';', $cliente), 3, null);
